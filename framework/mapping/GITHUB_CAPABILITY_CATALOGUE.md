...
**Out of scope:** GitHub Marketplace apps and third-party GitHub Apps; GitHub Copilot; API-only capabilities without a UI surface; Enterprise-server-only administration. **One deliberate exception:** the GitHub–Zenodo release webhook (GC61) is a third-party service but is included because it is the canonical bridge for persistent identification and preservation, and it connects to the prior GitHub–Zenodo–ORCID workflow.
