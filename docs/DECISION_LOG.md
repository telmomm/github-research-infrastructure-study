...
## D29 — Parallel manuscript for IEEE Computing in Science & Engineering (CiSE)

- **Date:** 2026-09-15 · **Closes:** GitHub issue #21
- **Context:** `v1.1.0` (`D28`) is the *Scientometrics* submission snapshot: `sn-jnl` template, ~4.9k-word body, 62 references, structured Methods/Results/Discussion sections. A parallel submission to *Computing in Science & Engineering* (IEEE Computer Society) targets a different readership and format: the `IEEEcsmag` magazine template, a ~7,200-word all-in budget (abstract + body + references + bio, each figure/table costing 300 words), a ≤20-reference cap, and narrative, jargon-light prose rather than journal section headers.
- **Decision:** Create a **second, sibling manuscript** at `manuscript_csmag/` rather than modifying or replacing `manuscript/`. It carries the same study, findings and figures, rewritten (not mechanically reformatted) to the magazine's register: abstract without citations, "How I Studied This / What I Found / What This Means" in place of Methods/Results/Discussion, superscript IEEE-style citations, references trimmed from 62 to the 20 most load-bearing, figures/tables reduced from 3+5 to 3+2, and an author biography appended. Figures and the `.cls`/`.sty` support files are flattened to the same directory as `paper.tex` (no subfolders) per IEEE ScholarOne submission convention.
- **Rationale:** the two venues' author guidelines are close to mutually exclusive on length and citation count — satisfying both from one source file would either violate CiSE's limits or strip *Scientometrics*' methodological depth. Two independently versioned manuscripts, each traceable to its own venue's requirements, is more honest than a single compromise draft, and mirrors how this repository already treats `manuscript/` as a versioned research output (git history + a Zenodo-archived release) rather than a single mutable file.
- **Verified:** `manuscript_csmag/paper.tex` compiles clean with `pdflatex` (2 passes, 0 undefined references), 6 pages; body+abstract+references ≈3,042 words (figures/tables/captions excluded per CiSE's counting rule), within the ~7,200-word budget; 3 figures, 2 tables, 20 references.
- **Not done:** actual submission to ScholarOne; a decision on whether `manuscript_csmag/` gets its own Zenodo/CITATION.cff entry is deferred until the CiSE submission is confirmed.
- **Affects:** new `manuscript_csmag/` (`paper.tex`, `IEEEcsmag.cls`, `sfmath.sty`, `upmath.sty`, `coword_map.pdf`, `fig3_architecture.png`, `fig4_traceability.png`, `paper.pdf`); `.gitignore` (excludes the local `manuscript/TEMPLATE - CsMag/` vendor download, reference-only); `CHANGELOG.md`.

## D30 — CiSE editorial feedback: references cut from 20 to 12

- **Date:** 2026-09-23 · **Refines:** `D29`
- **Context:** the CiSE editorial office returned the manuscript: "Your article currently has too many references. Please adhere to the limit of 12 references." `D29` had assumed a ≤20 cap.
- **Decision:** trim `manuscript_csmag/` to 12 references, one or two per literature strand the introduction names: workflows (Liew 2016), research data management (Perrier 2017), post-award management (Crane 2023), provenance (Davidson & Freire 2008), process lifecycle models (Cox & Tam 2018), research information systems (Haris 2026), open science (Klebel 2025), Git/GitHub guidance in computational biology, ecology and the lab (Blischak 2016, Braga 2023, Chen 2025), GitHub in the scholarly record (Escamilla 2022) and its critical counterpoint (Kalliamvakou 2016). Dropped: Mattoso 2015, Pérez 2018, Kirkham 2020, Mayernik 2017, Devkota 2026, Sandve 2013, Wilson 2017, Crystal-Ornelas 2021. Citations renumbered in order of first appearance; this also fixes several superscripts in the `D29` draft that pointed at the wrong entries (e.g. the provenance strand cited Kirkham's preprint-platform review). One sentence reworded ("scholarly infrastructure and grant reporting" → "post-award grant management") to match the remaining source.
- **Verified:** `pdflatex` 2 passes, clean, 6 pages; 12 `\bibitem`s, all cited, numbered sequentially.
- **Affects:** `manuscript_csmag/paper.tex`, `manuscript_csmag/paper.pdf`, `CHANGELOG.md`.

## D31 — CiSE editorial feedback: abstract cut to ≤150 words

- **Date:** 2026-09-24 · **Refines:** `D29`, `D30`
- **Context:** the CiSE editorial office unsubmitted CiSE-2026-09-0146 a second time: "Your abstract is currently over the 150-word limit." The `D29`/`D30` abstract was 189 words.
- **Decision:** rewrite the abstract to 141 words, keeping every claim (problem, question, method, 13/15 and 7/15 coverage, upstream gap, architecture + template, case study + retrospective transfer check, positioning). Dropped only secondary detail: the list of software-engineering-like areas, "fifteen-component", "machine-learning", "this project's own repository". Em-dash asides replaced by parentheses so a portal that counts `---` as a token still lands well under 150. Guidelines re-checked: total ≈4,700 of the 7,200-word budget (figures/tables at 300 each), 12 references, 3 figures + 2 tables.
- **Verified:** `pdflatex` 2 passes, clean, 6 pages; `manuscript_csmag.zip` rebuilt.
- **Affects:** `manuscript_csmag/paper.tex`, `manuscript_csmag/paper.pdf`, `manuscript_csmag.zip`, `CHANGELOG.md`.
